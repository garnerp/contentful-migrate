...
describe('Integration Test', () => {
  ...
  describe('up command', () => {
    ...
    it('-a should run all content types not migrated @integration', async () => {
      const previousContentType = 'horse';
      const contentType = 'alpaca';

      const previousMigrations = await client.getSpace(spaceId)
        .then(space => space.getEnvironment(environmentId))
        .then(environment => environment.getEntries({
          content_type: 'migration',
          'fields.contentTypeId': previousContentType
        }));
      ...
      const updatedPreviousMigrationLastRun = migrations.items.find((item) => {
        return item.fields.contentTypeId[defaultLocale] === previousContentType;
      }).fields.state[defaultLocale].migrations[0].timestamp;
      expect(updatedPreviousMigrationLastRun).to.be(previousMigrationsLastRun);
    }).timeout(30000);
  });
});
